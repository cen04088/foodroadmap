...
BROADCASTS_HTML_WITH_EXCLUDED_NAME = """
<a class="lp-net__card" href="/broadcast/ttoganjib">
  <span class="lp-net__meta"><span class="lp-net__name">또간집</span></span>
</a>
<a class="lp-net__card" href="/broadcast/renamed-slug">
  <span class="lp-net__meta"><span class="lp-net__name">동네한바퀴</span></span>
</a>
"""


def test_run_crawl_skips_excluded_broadcasts_by_name_even_if_the_slug_changed():
    session_factory = make_in_memory_session_factory()

    def fake_fetch(url, **kwargs):
        if url == f"{BASE_URL}/broadcasts":
            return BROADCASTS_HTML_WITH_EXCLUDED_NAME
        if url.startswith(f"{BASE_URL}/broadcast/ttoganjib"):
            return LIST_PAGE_HTML
        if url == f"{BASE_URL}/place/place-1":
            return DETAIL_HTML
    ...
    with session_factory() as session:
        assert session.get(Broadcast, "ttoganjib") is not None
        assert session.get(Broadcast, "renamed-slug") is None
